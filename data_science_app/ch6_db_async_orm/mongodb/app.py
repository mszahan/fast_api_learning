...
async def get_post_or_404(id: ObjectId = Depends(get_object_id),
                          databse: AsyncIOMotorDatabase = Depends(get_database)) -> Post:
    raw_post = await databse['posts'].find_one({'_id': id})
    if raw_post is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
...
async def create_post(post_create: PostCreate,
                      database: AsyncIOMotorDatabase = Depends(get_database)) -> Post:
