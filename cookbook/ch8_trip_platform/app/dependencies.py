...
def check_date_condition(start: date, end: date):
    if end and end < start:
        raise HTTPException(
            status_code=400, detail="End date must greater than start date")
...
def time_range(
    ...
    check_date_condition(start, end)
    return start, end


def select_category(category: Annotated[str, Path(
