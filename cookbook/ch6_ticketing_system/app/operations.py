...
async def create_event(
    ...
    async with db_session.begin():
        ...
        await db_session.flush()
        event_id = event.id
        tickets = [Ticket(show=event_name,
...
async def get_event_sponsorships_with_amount(
    ...
    query = select(Sponsor.name, Sponsorship.amount).join(
        Sponsorship, Sponsorship.sponsor_id == Sponsor.id,
    ).where(Sponsorship.event_id == event_id).order_by(Sponsorship.amount.desc())
    async with db_session as session:
        result = await session.execute(query)
...
async def get_events_tickets_with_user_price(
    ...
    query = select(Ticket).where(Ticket.event_id == event_id).options(
        load_only(Ticket.id, Ticket.user, Ticket.price)
    )
    async with db_session as session:
        result = await session.execute(query)
        tickets = result.scalars().all()
...
async def sell_ticket_to_user(
    ...
    ticket_query = update(Ticket).where(and_(
        Ticket.id == ticket_id, Ticket.sold == False)).values(
        user=user, sold=True
    )
    async with db_session as session:
        result = await session.execute(ticket_query)
